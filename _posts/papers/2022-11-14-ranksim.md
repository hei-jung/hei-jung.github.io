...
예를 들어..
age estimation example에서 $ z^n $: label space에서 n살에 해당하는 learned feature, $ \sigma(\dot, \dot) $: 두 벡터 간 similarity function (예: cosine 유사도) 라고 할 때, 1, 21, 25, 70살에 대한 이미지 데이터를 가지고 있다고 하자.
